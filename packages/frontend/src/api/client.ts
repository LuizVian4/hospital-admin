...
export const api = {
  ...
  downloadEscalaExcel: async (competenciaId: number) => {
    const res = await fetch(`${API_URL}/api/competencias/${competenciaId}/escala/export`);
    if (!res.ok) {
      const err = await res.json().catch(() => ({ error: res.statusText }));
      throw new Error(err.error || `HTTP ${res.status}`);
    }
    ...
    const filename = match?.[1] ?? `escala-${competenciaId}.xlsx`;
  ...
  trocarEscalaDia: (competenciaId: number, data: TrocaEscalaRequest) =>
    request<{ success: boolean }>(`/api/competencias/${competenciaId}/troca`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
